...
object CustomMapRules {
    ...
    fun validationError(
        ...
        val minimumPlayableCells = minOf(MIN_PLAYABLE_CELLS, rows * columns - 1)
        if (playableCount < minimumPlayableCells) {
            return "Keep at least $minimumPlayableCells playable cells."
        }
        val minimumPlayablePerLine = minOf(MIN_PLAYABLE_PER_LINE, rows - 1, columns - 1)
        val sparseRow = (0 until rows).any { row ->
            (0 until columns).count { col -> Cell(row, col) !in blockedCells } < minimumPlayablePerLine
        }
        val sparseColumn = (0 until columns).any { col ->
            (0 until rows).count { row -> Cell(row, col) !in blockedCells } < minimumPlayablePerLine
        }
        if (sparseRow || sparseColumn) {
            return "Keep at least $minimumPlayablePerLine playable cells in every row and column."
        }
        return null
    }
