...
internal fun GridfallColors.isFrutigerAeroTheme(): Boolean = this == FrutigerAeroColors

internal fun Modifier.frutigerAeroAppTexture(colors: GridfallColors): Modifier {
    if (!colors.isFrutigerAeroTheme()) return this
...
internal fun Modifier.frutigerAeroBoardFrameTexture(colors: GridfallColors): Modifier {
    if (!colors.isFrutigerAeroTheme()) return this
    return drawWithCache {
        onDrawBehind {
...
internal fun Modifier.frutigerAeroBoardWellTexture(colors: GridfallColors): Modifier {
    if (!colors.isFrutigerAeroTheme()) return this
    return drawWithCache {
        onDrawBehind {
...
internal fun DrawScope.drawFrutigerAeroEmptyCell(topLeft: Offset, cellSize: Float, colors: GridfallColors) {
